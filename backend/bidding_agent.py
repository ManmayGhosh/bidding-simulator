...
class BiddingAgent:
    def __init__(self, agent_id, budget=100000000.0, bias_type=None):
        ...
        # Initialize history with Point 0
    ...
    def update_stats(self, cost, clicked, observation, is_winner):
        """Every agent calls this to keep history counts perfectly synced."""
        if is_winner:
            ...
            return {
                ...
                "views": total_views, "clicks": predicted_clicks, "interval": interval
            }
        else:
            # Losers still record a point to maintain graph continuity
            self.history.append({"budget": self.budget, "profit": self.cumulative_profit, "clicks": self.clicks})
            return None
